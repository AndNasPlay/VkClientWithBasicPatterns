//
//  UIFontExtension.swift
//  VkClientWithBasicPatterns
//
//  Created by Андрей Щекатунов on 30.09.2021.
//

import UIKit

extension UIFont {
	static let titleCellFont: UIFont = .systemFont(ofSize: 16, weight: .regular)
	static let subTitleCellFont: UIFont = .systemFont(ofSize: 13, weight: .regular)
	static let profileNameFont: UIFont = .systemFont(ofSize: 20, weight: .bold)
	static let profileStatusFont: UIFont = .systemFont(ofSize: 15, weight: .regular)
	static let profileActiveStatusFont: UIFont = .systemFont(ofSize: 14, weight: .regular)
	static let iconsFont: UIFont = .systemFont(ofSize: 8, weight: .regular)
}
